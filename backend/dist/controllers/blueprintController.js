...
exports.uploadBlueprint = uploadBlueprint;
exports.deleteBlueprint = deleteBlueprint;
exports.getUserBlueprints = getUserBlueprints;
exports.getBlueprintById = getBlueprintById;
exports.updateBlueprint = updateBlueprint;
exports.analyzeExistingBlueprint = analyzeExistingBlueprint;
exports.testAIService = testAIService;
exports.getUserAIUsage = getUserAIUsage;
 ...
 */
async function getUserObjectId(firebaseUid, userInfo) {
...
async function uploadBlueprint(req, res) {
    try {
        ...
        if (!(0, s3Service_1.validateS3Config)()) {
        ...
        // Upload to S3
        const uploadResult = await (0, s3Service_1.uploadToS3)(req.file.buffer, req.file.originalname, req.file.mimetype, 'blueprints');
        // Create blueprint document
        const blueprint = new Blueprint_1.Blueprint({
        ...
        if (projectId) {
            try {
                const project = await Project_1.Project.findOne({ _id: projectId, userId: userObjectId });
                if (project) {
                    const blueprintObjectId = blueprint._id;
        ...
        processAIAnalysis(blueprint._id.toString(), req.file.buffer, req.file.mimetype, userObjectId)
            .catch(error => {
            console.error('Background AI analysis failed:', error);
        });
        const responseData = {
            _id: blueprint._id.toString(),
            id: blueprint._id.toString(), // Add both _id and id for compatibility
            name: blueprint.name,
...
async function deleteBlueprint(req, res) {
    try {
        ...
        const blueprint = await Blueprint_1.Blueprint.findOne({ _id: id, userId: userObjectId });
        if (!blueprint) {
        ...
        // Delete from S3
        if (blueprint.s3Key) {
            await (0, s3Service_1.deleteFromS3)(blueprint.s3Key);
        }
        // Delete from database
        await Blueprint_1.Blueprint.findByIdAndDelete(id);
        res.json({
            success: true,
 ...
 */
async function getUserBlueprints(req, res) {
    try {
        const firebaseUid = req.user?.uid;
        if (!firebaseUid) {
            return res.status(401).json({
        ...
        const total = await Blueprint_1.Blueprint.countDocuments(filter);
        ...
        let queryBuilder = Blueprint_1.Blueprint.find(filter).sort({ createdAt: -1 });
        if (limitNumber !== null) {
...
async function updateBlueprint(req, res) {
    try {
        ...
        if (totalSymbols !== undefined)
            updateData.totalSymbols = totalSymbols;
        if (averageAccuracy !== undefined)
            updateData.averageAccuracy = averageAccuracy;
        const blueprint = await Blueprint_1.Blueprint.findOneAndUpdate({ _id: id, userId: userObjectId }, updateData, { new: true, runValidators: true });
        if (!blueprint) {
 ...
 */
async function processAIAnalysis(blueprintId, imageBuffer, mimeType, userId) {
    try {
        ...
        const validation = (0, aiService_1.validateImageForAnalysis)(imageBuffer, mimeType);
        if (!validation.valid) {
        ...
        if (userId) {
            const canAnalyze = await AIUsage_1.AIUsage.canUserAnalyze(userId);
        ...
        const analysisResult = await (0, aiService_1.analyzeBlueprint)(imageBuffer, mimeType);
        ...
        console.log(`📝 Converted to ${blueprintSymbols.length} blueprint symbols with percentage coordinates:`, blueprintSymbols.map(s => ({ name: s.name, confidence: s.confidence, position: s.position })));
        // Update blueprint with AI results
        console.log(`💾 Updating blueprint ${blueprintId} with analysis results...`);
        const updatedBlueprint = await Blueprint_1.Blueprint.findByIdAndUpdate(blueprintId, {
        ...
        if (userId && updatedBlueprint) {
            const cost = (0, aiService_1.getAnalysisCost)(imageBuffer.length);
            await AIUsage_1.AIUsage.recordAnalysis(userId, cost);
        ...
        await Blueprint_1.Blueprint.findByIdAndUpdate(blueprintId, {
            status: 'failed',
 ...
 */
async function analyzeExistingBlueprint(req, res) {
    try {
        const { id } = req.params;
        const firebaseUid = req.user?.uid;
        if (!firebaseUid) {
        ...
        const canAnalyze = await AIUsage_1.AIUsage.canUserAnalyze(userObjectId);
        if (!canAnalyze.canAnalyze) {
 ...
 */
async function testAIService(req, res) {
    try {
        console.log('🧪 Testing AI service health...');
        const healthCheck = await (0, aiService_1.checkAIServiceHealth)();
        ...
        const cost = (0, aiService_1.getAnalysisCost)(testImageBuffer.length);
        res.json({
            success: true,
 ...
 */
async function getUserAIUsage(req, res) {
    try {
        const firebaseUid = req.user?.uid;
        if (!firebaseUid) {
            return res.status(401).json({
        ...
        const usage = await AIUsage_1.AIUsage.getUserUsage(userObjectId);
        const canAnalyze = await AIUsage_1.AIUsage.canUserAnalyze(userObjectId);
        res.json({
            success: true,
            data: {
